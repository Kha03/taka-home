...
export default function ContractsPage() {
    ...
    async (contractId: string, invoiceId?: string) => {
      try {
        ...
        if (invoices && invoices.length > 0) {
          ...
          alert(t("noInvoiceFound"));
        }
      } catch (error) {
        console.error("Error viewing invoice:", error);
        alert(t("cannotLoadInvoice"));
      }
    },
    [actions]
  );
  ...
  const contractTabs = useMemo(() => {
    ...
    return [
      ...
      { id: "active", label: t("active"), count: activeCount },
      { id: "expired", label: t("expired"), count: expiredCount },
    ];
  }, [contracts]);
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {loading ? (
          <div className="text-center py-12">
            <LoadingSpinner size="xl" text={tCommon("loading")} />
          </div>
        ) : pageItems.length === 0 ? (
          <div className="text-center py-12">
            <Card className="bg-primary-foreground p-8">
              <CardContent>
                ...
                <p className="text-gray-500">
                  {searchQuery ? t("tryChangeKeyword") : t("noContractsYet")}
                </p>
              </CardContent>
            </Card>
          </div>
